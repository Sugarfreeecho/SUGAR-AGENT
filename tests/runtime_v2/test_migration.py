...
def test_migration_service_exports_v2_ui_and_model_projection_to_legacy(tmp_path):
...
def test_migration_service_backfills_v2_from_legacy_only_when_explicit(tmp_path):
    saved_ui = []
    saved_model = []
    ...
    assert saved_ui == []
    assert saved_model == [[{"type": "user", "content": "legacy"}]]
