...
| 模式 | 执行边界 | 审批 |
|------|------|------|
| 请求批准 | `app_restricted` | 普通只读（含工作区外）、工作区内写入与普通 Shell 自动放行；工作区内删除为普通黄色审批，敏感文件读取、工作区外写入/Shell、网络、不可恢复删除或未知副作用需审批 |
| 替我审批 | 与“请求批准”完全相同 | 独立只读审查 Agent 决定是否批准 |
| 完全访问权限 | 普通操作 `no_restriction` | 普通文件、工作区内删除、Shell 与网络操作免审批；格式化、磁盘写入、关机等非删除高危命令仍需一次性人工确认，Agent 自保与控制器完整性红线始终拒绝 |

`app_restricted` 使用当前操作系统用户运行，通过中央能力策略、工作区路径校验、敏感环境过滤、危险操作检查和摘要绑定审批限制工具。它是应用层防护，不是硬沙箱；不会因为缺少原生 OS 沙箱而降级只读。普通读取（含工作区外）自动允许，凭据/信息安全类文件读取需审批；工作区内普通写入和 Shell 自动允许；工作区内删除与动态代码为普通黄色审批，可由自动审查或任务授权处理，不强制人工弹窗；格式化、磁盘写入、关机等非删除高危命令为红色强制审批，凭据导出与安全策略篡改默认拒绝。安全策略、全局权限档位、审批记录、插件信任和 MCP 注册记录保存在工作区外。审批 grant 仍绑定具体会话与请求摘要；权限模式本身不做会话隔离。

`full_access` 取消普通操作（包括工作区内删除）的应用层限制与逐项审批，但不取消安全红线：格式化、磁盘写入、关机等非删除高危 Shell 命令在所有模式下都必须取得与最终请求摘要绑定的一次性人工授权；终止 Agent、沿父进程链杀进程、重启运行中的控制器或删除其运行文件等操作在所有模式下直接拒绝。

审批卡片提供**“替我分析”**：复用审查模型给出风险等级、分析理由与允许/拒绝建议，但不代行审批；拒绝操作时会优先携带自动审批或“替我分析”的拒绝理由，没有可用审查结论时要求用户填写拒绝原因，并将原因返回给 Agent；工作区外操作与工具执行**分开审批**——先审批工作区处理，之后仍需单独审批工具操作。审查上下文携带待审批请求与用户意图，仅在审批期间保留于内存。
...
│       │   ├── config.js         # 运行时配置读取
│       │   └── modules/          # 功能模块
│       │       ├── message-rendering.js      # 消息与执行过程渲染（媒体渲染）
...
| 变量 | 默认 | 说明 |
|------|------|------|
| `SECURITY_ENABLED` | `1` | 普通安全审批开关（`0` 进入完全访问；破坏性审批与 Agent 自保红线仍保留） |
