...
function lineChart(target, rows, series, valueSuffix='ms') {
  ...
  svg+='</svg><div class="legend">'+series.map((s,i)=>`<span><i style="background:${colors[i%colors.length]}"></i>${esc(s.name)}</span>`).join('')+'</div>';
  const previous=chartScrollState.get(target);el.innerHTML=svg;
  requestAnimationFrame(()=>{
    const maxScroll=Math.max(0,el.scrollWidth-el.clientWidth);
    el.scrollLeft=previous==null||previous.atEnd?maxScroll:Math.min(maxScroll,previous.left);
  });
  el.onscroll=()=>chartScrollState.set(target,{left:el.scrollLeft,atEnd:el.scrollLeft>=el.scrollWidth-el.clientWidth-2});
}
...
function renderCharts(rows){
  ...
  lineChart('phase-chart',rows,phaseNames.slice(0,8).map(name=>({name,value:r=>num((r.req.phases[name]||{}).total_ms)})));
}
...
function phaseHtml(session,run,req,name,phase){
  ...
  return `<details class="phase" data-open-key="${esc(key)}" ${opened?'open':''}><summary><span>${esc(name)}</span><b>${esc(ms(phase.total_ms))}</b></summary><div>${eventsHtml(phase.events)}</div></details>`;
}
