...
状态并记录净变更。默认关闭文件审计；开启后可使用 Git 状态或完整文件系统快照，
其中 `full` 模式还能发现 ignored 文件和非 Git 目录中的创建、修改和删除。

这是工具调用边界的净变更审计，不是内核级文件追踪：一次调用中先创建后删除的瞬时文件，
以及工作区之外未声明的外部路径，不一定能被轨迹捕获。可用 `FILE_AUDIT_MODE` 选择审计
模式：`off` 关闭文件审计（不采集任何状态，默认）、`git` 仅使用 Git 工作区状态、
`full` 遍历整个工作区以覆盖 ignored 和非 Git 文件；完整遍历可用
`FILE_AUDIT_MAX_FILES` 设置文件数上限。未设置 `FILE_AUDIT_MODE` 时默认 `off`；
旧开关 `FILE_AUDIT_FULL_SNAPSHOT` 已不再支持，设置后也不会生效。
