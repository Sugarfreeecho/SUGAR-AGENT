...
- **预期现象**：优先调用 `history_context(action="search", scope="current")` 搜索当前会话；拿到稳定 `ref` 后用 `action="read"` 分页读取。只有明确需要跨会话时才使用 `scope="global"`；工具未命中时可按返回的 `source_file` 检查原始 `events.jsonl`。
- **规则与边界**：`read` 的当前会话作用域不得越权读取其他会话；单页最多返回 50,000 字符；搜索与读取均为只读宿主工具。活跃摘要只负责提示线索，归档和事件流才是细节证据。
...
| 摘要中的引用无法直接找回原文 | 新增 `history_context`，支持当前会话/全局会话搜索、稳定引用读取和分页；保留 `source_file` 供原文件兜底检查。 |
