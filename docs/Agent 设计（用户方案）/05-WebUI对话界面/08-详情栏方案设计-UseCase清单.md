...
- 2026-09-20 v14：**窄窗零占用（补全）**——在 v13"变窄自动收起"之外，补齐两条残余路径：①半屏里"手动点开 / 点文件链接 / 点改动审查-查看"要打开详情栏时，自动改用**全屏覆盖**模式（覆盖而不推挤，关闭后工作区原样）；②切换会话时若"进入的会话详情栏是打开状态"且窗口放不下，保持**收起**。查证结论：代码中**不存在"窗口变窄就自动打开详情栏"的逻辑**，所有打开路径均为用户动作或会话记忆。冒烟 54 项全绿（新增：窄窗关闭态工作区满宽、窄窗手动打开自动全屏覆盖、窄窗文件链接自动全屏覆盖）。
- 2026-09-19 v13：**窄窗自动收起（半屏默认收起）**——窗口收窄到"详情栏打开会把主工作区压到低于 560px 可用宽度"时，详情栏自动收起让位，而不是继续推挤把工作区挤成一条缝；窗口再变宽不会自动弹开（保持收起，用户可随时点右上角按钮打开）。冒烟 51 项全绿（新增"窄窗自动收起"断言：1000px 窗口下 `collapsed=true、main=780`）。
- 2026-09-15 v12：**全屏切换改为瞬时**——全屏/退出全屏不再走宽度/位移过渡（切换窗口期加 `is-instant` 禁过渡，两帧后移除）；推挤模式的开/合动画保持不变。**标题栏副标题（工作区路径 + 会话 ID）窄窗单行截断**——窗口贴半屏时不再因 `word-break: break-all` 换行占满整屏（改为 nowrap + ellipsis，完整值仍可悬停查看）。冒烟 50 项全绿（含 520px 窄窗单行断言；并把"Failed to fetch"类后台网络抖动从页面错误断言中排除，脚本错误仍判失败）。
- 2026-09-15 v11：**Markdown 在详情栏渲染**——`.md/.markdown` 打开默认走应用自带的 `renderMarkdown`（与聊天同一条解析管线，视觉一致），头带"查看源码/预览文档"一键切换、刷新重读；Markdown 内的相对链接与图片改写为 `/api/workspace-assets/<dir>/…` 使其可解析；文件窗口右上角新增**「打开文件目录」**图标按钮（图形逐字取自 dsh `IconFolderOpenOutline16`；新路由 `GET /api/open-workspace-dir`，实现与"打开会话目录/打开工作目录"链接**同一机制**：把所在目录交给平台打开器 `os.startfile / open / xdg-open`——此前 Windows 试过 `explorer /select` 高亮文件，但子进程传参下会被 explorer 误解析并回落到系统"文档"目录，故按用户要求与既有入口保持一致；路径同样经允许根校验，`probe=1` 只校验不弹窗供测试）；失败提示带具体原因；冒烟 49 项全绿。⚠️ 目录路由需重启服务一次生效。
- 2026-09-15 v10：**子代理寻址保持详情栏**——`dockRightSurfaceKey` 把子会话 id 归一到父会话（依据 `subagentAddressing` 栈与 `subagentCatalogStore` 地址），进入/返回子代理不再收起详情栏或切到空面；公开 API 增加 `surfaceKey()`；冒烟 44 项全绿。
