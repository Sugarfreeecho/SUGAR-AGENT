...
- 版本：2026-09-20 v5（补录子代理任务生命周期交叉引用；覆盖至：当前工作区）
- 用途：逐条审查（四字段格式）。
...
- 子代理的**任务生命周期与事件循环隔离**（后台托管、跨循环等待/取消、事件与结果通道）见 [../02-ReAct运行时/10-子代理任务生命周期与事件循环隔离方案设计-UseCase清单.md](../02-ReAct运行时/10-子代理任务生命周期与事件循环隔离方案设计-UseCase清单.md)；
