...
- **规则与边界**：更新是"单节替换"语义（append 会炸上下文，已被设计排除）；旧摘要可追溯（事件流里有 body 记录）；本地摘要还携带压缩原文归档引用，细节可通过 `history_context` 找回。
...
- 活跃上下文中的摘录只保留可容纳的用户/助手正文，其余以稳定引用占位；完整工具结果、reasoning 等保存在压缩归档或原始事件流中。
- 回溯压缩内容时先用 `history_context(scope="current")`，只有明确跨会话需求才扩大到 `scope="global"`；不得仅凭摘要补猜细节。
- 压缩算法本体见 ../09-横切能力/01。
...
- 2026-09-20 v2：补充压缩归档引用、活跃摘录与 `history_context` 回溯边界。
- 2026-09-13 v1：拆分首版。
