...
- 普通读取（含工作区外）默认放行，凭据/信息安全类文件（`.env`、`.ssh`、密钥、凭据库等）读取需审批；工作区内普通写入和 Shell 自动允许；工作区外写入、Shell 外部路径、网络、不可恢复删除及未知副作用请求审批；动态代码（`-c/-e`、编码命令、`eval` 等）为普通黄色审批，可“本次/本任务”放行，但不可沉淀“始终允许”规则；破坏性命令为红色强制审批；`delete_file` 为可恢复软删除（移入 `WORK_DIR/.trash/`），工作区内外均自动放行；凭据导出和安全策略篡改默认拒绝。
...
- 完全访问无需设置页预先启用；用户切换时必须看到强警告，并明确提示该全局状态会跨任务、工作区和应用重启保持。
