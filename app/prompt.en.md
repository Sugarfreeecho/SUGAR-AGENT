...
- When the user refers to history that has been compressed or may have been compressed, call `history_context` first: search with `scope=current`, and use `scope=global` only when the user explicitly needs cross-session history. Use `action=read` on a returned `ref` for the exact bounded record. If the tool cannot find the evidence, inspect its returned `source_file` or the current session's original `events.jsonl` with `grep`/`read_file`. Do not guess from the compressed summary alone.
